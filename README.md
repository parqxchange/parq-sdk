...
Depends directly on `@solana/web3.js` ^1.98 and `@coral-xyz/anchor` ^0.31 — match these versions in your app to avoid duplicate web3.js instances. Node `>=18` for the indexer client (`AbortSignal.timeout`); discriminators are computed with `@noble/hashes`, so the main and `/minimal` entries carry no Node-only crypto — only `/node` requires Node.
