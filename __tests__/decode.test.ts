...
describe("decodePoolState — v4/v5/v6 length tolerance (2026-06-12 collateral-escrow realloc)", () => {
  ...
  it("rejects any other length (the regression that bricked /farm + closes)", () => {
    ...
    // and threw `got 260` for every pool. 268 is now VALID (v7 #12 builder_earmark
    // — migrate_pool_state_v4). Guard the old gap + an over-long that is NOT a
    // known layout (276 = 268 + 8).
    expect(() => decodePoolState(buildPoolState(248))).toThrow(DecoderError);
    expect(() => decodePoolState(buildPoolState(276))).toThrow(DecoderError);
  });

  it("accepts v7 (268) and decodes builderEarmarkUsdc (@260); defaults 0n on pre-v7", () => {
    const buf = buildPoolState(268);
    const view = new DataView(buf.buffer, buf.byteOffset, buf.byteLength);
    writeU64(view, 260, 7_777_000n); // builder_earmark_usdc
    const out = decodePoolState(buf);
    expect(out.builderEarmarkUsdc).toBe(7_777_000n);
    // pre-v7 (240/252/260) buffers default builderEarmarkUsdc to 0n.
    expect(decodePoolState(buildPoolState(240)).builderEarmarkUsdc).toBe(0n);
    expect(decodePoolState(buildPoolState(260)).builderEarmarkUsdc).toBe(0n);
  });
...
describe("decodeCategoryPool / decodeMarketRisk (Phase 0/3)", () => {
  it("decodeCategoryPool reads fields at the pinned offsets (LEN 264)", () => {
    ...
    // 260 is NOT a CategoryPool length (264/290/298 are); error names all three.
    { const bad = new Uint8Array(260); bad.set(disc("CategoryPool"), 0); expect(() => decodeCategoryPool(bad)).toThrow(/264, 290, or 298 bytes/); }
    // wd_bucket / earmark tail fields are undefined on the 264 layout.
    expect(cp.wdBucketAvailable).toBeUndefined();
    expect(cp.wdMaxBudgetBpsOfNav).toBeUndefined();
    expect(cp.builderEarmarkUsdc).toBeUndefined();
  });

  it("decodeCategoryPool reads the #7 wd_bucket (@264..290) on a 290-byte buffer", () => {
    const a = new Uint8Array(290);
    a.set(disc("CategoryPool"), 0);
    a[255] = 3; // bump
    const v = new DataView(a.buffer);
    writeU64(v, 256, 66n);            // sum_side_bucket
    writeU64(v, 264, 12345n);         // wd_bucket_available
    writeU64(v, 272, 99n);            // wd_bucket_last_slot
    v.setUint16(280, 2500, true);     // wd_max_budget_bps_of_nav
    writeU64(v, 282, 7n);             // wd_refill_per_slot
    const cp = decodeCategoryPool(a);
    expect(cp.sumSideBucketUsdc).toBe(66n);
    expect(cp.wdBucketAvailable).toBe(12345n);
    expect(cp.wdBucketLastSlot).toBe(99n);
    expect(cp.wdMaxBudgetBpsOfNav).toBe(2500);
    expect(cp.wdRefillPerSlot).toBe(7n);
    // builder_earmark is still absent on the #7-only (290) layout.
    expect(cp.builderEarmarkUsdc).toBeUndefined();
  });

  it("decodeCategoryPool reads the #12 builder_earmark (@290..298) on a 298-byte buffer", () => {
    const a = new Uint8Array(298);
    a.set(disc("CategoryPool"), 0);
    a[255] = 3; // bump
    const v = new DataView(a.buffer);
    v.setUint16(280, 0, true);        // wd_max_budget_bps_of_nav = 0 (DISABLED, inert)
    writeU64(v, 290, 4_242_000n);     // builder_earmark_usdc
    const cp = decodeCategoryPool(a);
    expect(cp.wdMaxBudgetBpsOfNav).toBe(0);
    expect(cp.builderEarmarkUsdc).toBe(4_242_000n);
  });
