...
describe("decodeLiquidated (Phase 2 partial-liq fields)", () => {
  function baseLiquidatedBuf(len: number): Buffer {
    const buf = Buffer.alloc(len);
    eventDisc("Liquidated").copy(buf, 0);
    Buffer.alloc(32, 3).copy(buf, 8);     // owner
    Buffer.alloc(32, 4).copy(buf, 40);    // market_id
    buf.writeBigUInt64LE(0n, 72);         // bad_debt
    return buf;
  }

  it("decodes a widened 104-byte Liquidated with closedSize/remainingSize/feePaid", () => {
    const buf = baseLiquidatedBuf(104);
    buf.writeBigUInt64LE(2_611_940_299n, 80); // closed_size
    buf.writeBigUInt64LE(2_388_059_701n, 88); // remaining_size (> 0 ⇒ partial)
    buf.writeBigUInt64LE(1_671_641n, 96);     // fee_paid

    const ev = decodeAnchorEvent(buf.toString("base64"));
    expect(ev).not.toBeNull();
    expect(ev!.type).toBe("liquidated");
    if (ev!.type === "liquidated") {
      expect(ev.data.badDebt).toBe(0n);
      expect(ev.data.closedSize).toBe(2_611_940_299n);
      expect(ev.data.remainingSize).toBe(2_388_059_701n);
      expect(ev.data.feePaid).toBe(1_671_641n);
    }
  });

  it("decodes a legacy 80-byte Liquidated — Phase 2 fields default to 0n", () => {
    const buf = baseLiquidatedBuf(80);
    buf.writeBigUInt64LE(5_500_000n, 72); // bad_debt

    const ev = decodeAnchorEvent(buf.toString("base64"));
    expect(ev).not.toBeNull();
    expect(ev!.type).toBe("liquidated");
    if (ev!.type === "liquidated") {
      expect(ev.data.badDebt).toBe(5_500_000n);
      expect(ev.data.closedSize).toBe(0n);
      expect(ev.data.remainingSize).toBe(0n);
      expect(ev.data.feePaid).toBe(0n);
    }
  });
});
