...
// ---------------------------------------------------------------------------
// depositIx (Uniswap MINIMUM_LIQUIDITY dead-shares: lp_dead + system_program)
// ---------------------------------------------------------------------------

describe("depositIx", () => {
  const lpMint  = Keypair.generate().publicKey;
  const userLp  = Keypair.generate().publicKey;
  const lpDead  = Keypair.generate().publicKey;
  ...
  it("returns a TransactionInstruction with the pool program ID", async () => {
    const ix = await client.depositIx(depositAccounts, { amount: 100_000_000n, minLpOut: 0n });
    expect(ix).toBeInstanceOf(TransactionInstruction);
    expect(ix.programId.equals(POOL_PROGRAM_ID)).toBe(true);
  });

  it("discriminator matches global:deposit sha256[0..8]", async () => {
    const ix = await client.depositIx(depositAccounts, { amount: 100_000_000n, minLpOut: 0n });
    const expected = instructionDisc("deposit");
    expect(Array.from(ix.data.subarray(0, 8))).toEqual(Array.from(expected));
  });

  it("account order: poolState, usdcVault, vaultAuthority, lpMint, userLp, userUsdc, lpDead, depositor, tokenProgram, systemProgram", async () => {
    const ix = await client.depositIx(depositAccounts, { amount: 100_000_000n, minLpOut: 0n });
    ...
    expect(ix.keys[4]!.pubkey.equals(userLp)).toBe(true);
    expect(ix.keys[5]!.pubkey.equals(userUsdc)).toBe(true);
    expect(ix.keys[6]!.pubkey.equals(lpDead)).toBe(true);
    expect(ix.keys[7]!.pubkey.equals(user)).toBe(true);
    expect(ix.keys[8]!.pubkey.equals(TOKEN_PROGRAM_ID)).toBe(true);
    expect(ix.keys[9]!.pubkey.equals(SYSTEM_PROGRAM)).toBe(true);
  });
});

// ---------------------------------------------------------------------------
// sweepStrandedToInsuranceIx (admin recovery for the stranded-balance windfall)
// ---------------------------------------------------------------------------

describe("sweepStrandedToInsuranceIx", () => {
  ...
  it("returns a TransactionInstruction", async () => {
    const ix = await client.sweepStrandedToInsuranceIx(sweepAccounts, { amount: 1_000_000n });
    expect(ix).toBeInstanceOf(TransactionInstruction);
  });
  ...
  it("discriminator matches global:sweepStrandedToInsurance sha256[0..8]", async () => {
    const ix = await client.sweepStrandedToInsuranceIx(sweepAccounts, { amount: 1_000_000n });
    const expected = instructionDisc("sweepStrandedToInsurance");
    expect(Array.from(ix.data.subarray(0, 8))).toEqual(Array.from(expected));
  });

  it("account list order: poolState, poolVaultUsdc, vaultAuthority, lpMint, insuranceFund, insuranceVault, admin, tokenProgram", async () => {
    const ix = await client.sweepStrandedToInsuranceIx(sweepAccounts, { amount: 1_000_000n });
    expect(ix.keys.length).toBe(8);
    expect(ix.keys[0]!.pubkey.equals(poolState)).toBe(true);
    expect(ix.keys[1]!.pubkey.equals(poolVaultUsdc)).toBe(true);
    expect(ix.keys[2]!.pubkey.equals(vaultAuth)).toBe(true);
    expect(ix.keys[3]!.pubkey.equals(lpMint)).toBe(true);
    expect(ix.keys[4]!.pubkey.equals(insuranceFund)).toBe(true);
    expect(ix.keys[5]!.pubkey.equals(insuranceVault)).toBe(true);
    expect(ix.keys[6]!.pubkey.equals(admin)).toBe(true);
