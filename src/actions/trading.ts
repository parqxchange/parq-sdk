...
export async function openPosition(
  ...
  // WP-1: the Position PDA (and UserQueueClaims) seed on the OWNER, not the
  // signer — on-chain `seeds = [b"position", owner.key(), ...]`. For a
  // non-delegated open owner == signer (unchanged), but a delegated open MUST
  // derive these off the wallet owner or the PDA won't match on-chain.
  const ownerKey       = accounts.owner ?? accounts.signer;
  const [position]     = positionPDA(ownerKey, opts.marketId, args.positionNonce, opts.perpEngineId);
  ...
  const [defaultUserClaims] = userQueueClaimsPDA(opts.marketId, ownerKey, opts.poolProgramId);
  ...
  const ix = await opts.perpClient.openPositionIx(
    {
      ...
      owner: accounts.owner ?? accounts.signer,
...
export async function updateMargin(
  ...
  delta: bigint,
  /**
   * Unified-LP-pool (Phase 3) category accounts (see `openPosition`/`closePosition`).
   * When non-null the named pool/vault/vaultAuthority are swapped to the shared
   * CategoryPool set and the pinned marketRisk@[2]/categoryEngineAuth@[3] pair is
   * carried (via `categoryBuilderArg`). The category branch reads those for BOTH
   * the add and the remove leg, so the feeds (pinned at remaining_accounts[0]/[1])
   * are always passed — they come from `opts.{primary,secondary}FeedAccount`.
   * `engineAuth` STAYS the legacy per-market PDA (unused-for-signing on the category
   * branch — the category signer is `categoryEngineAuth` carried at RA[3]). Null/
   * undefined ⟹ legacy per-market shape (backward-compatible for existing callers).
   */
  category?: CategoryTradeAccounts | null,
): Promise<TransactionInstruction[]> {
  const [marketState]    = marketStatePDA(opts.marketId, opts.perpEngineId);
  ...
  const ix = await opts.perpClient.updatePositionMarginIx(
    {
      ...
      owner: accounts.owner,
      signerUsdc: accounts.signerUsdc,
      // Category: swap the named pool/vault/vaultAuthority to the shared
      // CategoryPool set (engineAuth stays the legacy per-market account).
      vaultUsdc: category ? category.categoryVault : vaultUsdc,
