...
/** Validate that the data is exactly the expected length. */
...
// unchanged. Accept BOTH lengths: a migrated pool is 252, any un-migrated
// legacy pool is 240 (ADL fields default to 0).

export function decodePoolState(data: Buffer | Uint8Array): PoolState {
  checkDisc(data, DISCRIMINATORS.PoolState, "PoolState");
  if (data.byteLength !== 240 && data.byteLength !== 252) {
    throw new DecoderError(
      `PoolState: expected 240 or 252 bytes, got ${data.byteLength}`
    );
  }
  const v = makeView(data);
