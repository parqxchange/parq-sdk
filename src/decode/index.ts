...
export const DISCRIMINATORS = {
  ...
  // #12 builder-fee primitive (Phoenix, 2026-06-24). Net-new accounts.
  BuilderRegistry:   sha256Disc("BuilderRegistry"),
  BuilderReward:     sha256Disc("BuilderReward"),
} as const;
...
//   --- V5 append (present when data.length >= 527) ---  (Phoenix #1 + #9, 2026-06-24)
//   [515..517] off_hours_band_bps: u16       ← #1 off-hours execution band (0 = disabled)
//   [517..525] last_external_anchor: u64      ← #1 relayer-stamped last-RTH-close (0 = unset)
//   [525..527] upnl_risk_factor_bps: u16      ← #9 asymmetric uPnL withdrawal haircut (0 = no haircut)
//
...
export function decodeMarketState(data: Buffer | Uint8Array): MarketState {
  ...
  // 515 (V4 — liq econ), 527 (V5 — #1 off-hours band + #9 uPnL haircut).
  // V5 appends off_hours_band_bps (2B @515) + last_external_anchor (8B @517) +
  // upnl_risk_factor_bps (2B @525) in one 515 → 527 realloc (migrate_market_state_v5).
  if (
    data.length !== 460 &&
    data.length !== 476 &&
    data.length !== 511 &&
  ...
  if (data.length >= 476) {
    ...
    if (data.length >= 511) {
      ...
      if (data.length >= 515) {
        ...
        // V5 append (515 → 527): #1 off-hours execution band + #9 uPnL withdrawal
        // haircut. Shipped inert (all three default 0); surfaced as optional so a
        // pre-v5 (515-byte) account decodes them as `undefined`.
        if (data.length >= 527) {
          return {
            ...v4,
            offHoursBandBps:    readU16LE(v, 515),
            lastExternalAnchor: readU64LE(v, 517),
            upnlRiskFactorBps:  readU16LE(v, 525),
          };
        }
        return v4;
      }
...
//   [260..268] builder_earmark_usdc: u64    (NEW v7 — #12 builder-fee earmark, APPENDED)
//
...
// v7: `migrate_pool_state_v4` (2026-06-24, #12) grows live pools 260→268,
// APPENDING builder_earmark_usdc after escrowed_collateral_usdc — offset-stable.
// Accept ALL FOUR lengths: a v7 pool is 268, a v6 pool is 260, a v5 pool is 252,
// any un-migrated legacy pool is 240 (newer fields default to 0). Mainnet pools
// were migrated to 260 on 2026-06-12; rejecting a known length bricks /farm +
// position closes. builder_earmark_usdc is 0 at #12 dark-launch.
...
export function decodePoolState(data: Buffer | Uint8Array): PoolState {
  ...
  if (
    ...
    throw new DecoderError(
      `PoolState: expected 240, 252, 260, or 268 bytes, got ${data.byteLength}`
    );
  }
  const v = makeView(data);
  const hasAdl = data.byteLength >= 252;
  const hasEscrow = data.byteLength >= 260;
  const hasEarmark = data.byteLength >= 268;
  return {
    marketId:           readBytes(v, 8, 32),
    ...
    builderEarmarkUsdc: hasEarmark ? readU64LE(v, 260) : 0n,
  };
}
...
// ---------------------------------------------------------------------------
//
// CategoryPool (LEN 264 base): disc[0..8] category_id[8..40] usdc_vault[40..72]
...
// #7 (290): APPENDS the withdrawal rate-limiter token bucket after
//   sum_side_bucket_usdc — wd_bucket_available[264..272] wd_bucket_last_slot[272..280]
//   wd_max_budget_bps_of_nav[280..282] wd_refill_per_slot[282..290].
// #12 (298): APPENDS builder_earmark_usdc[290..298] after wd_refill_per_slot.
// Accept ALL THREE lengths across the rolling migration window (264, 290, 298);
// `migrate_category_pool_wd_bucket` grows live pools 264→298 (also 290→298),
// seeding the #7 bucket DISABLED (bps=0, available=u64::MAX) and earmark=0 INERT.
export function decodeCategoryPool(data: Buffer | Uint8Array): CategoryPool {
  ...
  if (data.byteLength !== 264 && data.byteLength !== 290 && data.byteLength !== 298) {
    throw new DecoderError(
      `CategoryPool: expected 264, 290, or 298 bytes, got ${data.byteLength}`
    );
  }
  const v = makeView(data);
  const hasWdBucket = data.byteLength >= 290;
  const hasEarmark = data.byteLength >= 298;
  return {
    ...
    wdBucketAvailable:      hasWdBucket ? readU64LE(v, 264) : undefined,
    wdBucketLastSlot:       hasWdBucket ? readU64LE(v, 272) : undefined,
    wdMaxBudgetBpsOfNav:    hasWdBucket ? readU16LE(v, 280) : undefined,
    wdRefillPerSlot:        hasWdBucket ? readU64LE(v, 282) : undefined,
    builderEarmarkUsdc:     hasEarmark ? readU64LE(v, 290) : undefined,
  };
}
...
// ---------------------------------------------------------------------------
// BuilderRegistry decoder (#12 builder fee, Phoenix 2026-06-24)
// ---------------------------------------------------------------------------
//
// Byte layout (43 bytes total). Seeds: [b"builder", authority].
//   [0..8]   discriminator
//   [8..40]  authority: Pubkey      — the builder's signing wallet (claim signer)
//   [40..42] fee_bps: u16           — chosen surcharge bps (clamped to the on-chain cap)
//   [42]     bump: u8

export function decodeBuilderRegistry(data: Buffer | Uint8Array): BuilderRegistry {
  checkDisc(data, DISCRIMINATORS.BuilderRegistry, "BuilderRegistry");
  checkLen(data, 43, "BuilderRegistry");
  const v = makeView(data);
  return {
    authority: readPubkey(v, 8),
    feeBps:    readU16LE(v, 40),
    bump:      readU8(v, 42),
  };
}

// ---------------------------------------------------------------------------
// BuilderReward decoder (#12 builder fee, Phoenix 2026-06-24)
// ---------------------------------------------------------------------------
//
// Byte layout (81 bytes total) — verbatim AffiliateReward shape, different seeds.
...
export function decodeBuilderReward(data: Buffer | Uint8Array): BuilderReward {
  checkDisc(data, DISCRIMINATORS.BuilderReward, "BuilderReward");
  checkLen(data, 81, "BuilderReward");
  const v = makeView(data);
  return {
    builder:     readPubkey(v, 8),
    marketId:    readBytes(v, 40, 32),
    accruedUsdc: readU64LE(v, 72),
    bump:        readU8(v, 80),
  };
