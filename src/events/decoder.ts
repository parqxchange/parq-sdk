...
function sha256Disc(name: string): Buffer {
  return Buffer.from(sha256(new TextEncoder().encode(name))).subarray(0, 8) as Buffer;
}
...
export type DecodedEvent =
  ...
  | { type: "queueLpDraw";          data: QueueLpDrawEvent }
...
//
// PositionClosed — 96 bytes at launch; byte-additive appends since:
//   [8..40]    owner: Pubkey
//   [40..72]   market_id: [u8;32]
//   [72..80]   pnl: i64 SIGNED — GROSS price-move PnL
//   [80..88]   net_return: u64
//   [88..96]   exit_price: u64
//   [96..104]  funding_charge: i64   (#172 — absent on legacy events)
//   [104..112] collateral: u64       (#183 — net PnL = net_return − collateral)
//   [112..120] size_usdc: u64        (#183)
//   [120]      closed_by: u8         (#183 — 0 user / 1 keeper / 2 liquidation)
// Length-gated: short (legacy) events leave the trailing fields undefined.
//
function decodePositionClosed(data: Buffer): PositionClosedEvent {
  ...
  if (data.length >= 104) ev.fundingCharge = readI64LE(v, 96);
  if (data.length >= 112) ev.collateral    = readU64LE(v, 104);
  if (data.length >= 120) ev.sizeUsdc      = readU64LE(v, 112);
  if (data.length >= 121) ev.closedBy      = readU8(v, 120);
  return ev;
}

//
// Liquidated — 104 bytes total (8 disc + 96 payload) post-Phase-2:
//   [8..40]   owner: Pubkey
//   [40..72]  market_id: [u8;32]
//   [72..80]  bad_debt: u64
//   [80..88]  closed_size: u64     (Phase 2 partial liq, byte-additive)
//   [88..96]  remaining_size: u64  (0 ⇒ full close)
//   [96..104] fee_paid: u64
// Legacy 80-byte events (pre-Phase-2): new fields default to 0n.
//
function decodeLiquidated(data: Buffer): LiquidatedEvent {
  ...
  if (data.length >= 104) {
    return {
      ...base,
      closedSize:    readU64LE(v, 80),
      remainingSize: readU64LE(v, 88),
      feePaid:       readU64LE(v, 96),
    };
  }
  return { ...base, closedSize: 0n, remainingSize: 0n, feePaid: 0n };
}
...
//
// QueueLpDraw — 96 bytes total (8 disc + 88 payload). Followup #173.
//   [8..40]  market_id: [u8;32]
//   [40..48] idx: u64
//   [48..80] owner: Pubkey
//   [80..88] amount: u64
//   [88..96] total_usdc_after: u64
//
function decodeQueueLpDraw(data: Buffer): QueueLpDrawEvent {
  const v = makeView(data);
  return {
    marketId:       readBytes(v, 8, 32),
    idx:            readU64LE(v, 40),
...
export function decodeAnchorEvent(base64Data: string): DecodedEvent | null {
  ...
  if (discMatches(raw, EVENT_DISCS.QueueLpDraw)) {
    ...
    if (data) return { type: "queueLpDraw", data };
    return null;
  }
