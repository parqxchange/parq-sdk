...
// ---------------------------------------------------------------------------
// Laddered-exit primitive (Spec #15 §4B — OFF-CHAIN, no redeploy)
// ---------------------------------------------------------------------------

/** One rung of a laddered TP/SL exit (a single decrease order). */
export interface ExitRung {
  /** Mark trigger price for this rung. */
  triggerPrice: bigint;
  /** Partial size for this rung; sum across rungs must be <= the position size. */
  sizeUsdc: bigint;
  /**
   * Optional explicit per-rung slippage floor (post-fee/funding NET-payout
   * floor checked on-chain by execute_order's existing `min_output_usdc`
   * guard). When set it WINS over the default band (incl. `0n` to disable the
   * floor entirely). When omitted, the default band (tight TP / wide SL) is
   * applied against `expectedNetPayoutUsdc` if that is supplied; absent both,
   * the floor is `0n` (today's no-floor behavior).
   */
  minOutputUsdc?: bigint;
  /**
   * Optional expected NET payout for this rung (current mark − fees − funding),
   * used ONLY to derive the default-band floor when `minOutputUsdc` is omitted.
   * The builder never fetches chain state, so the caller supplies this.
   */
  expectedNetPayoutUsdc?: bigint;
}

/** Tight default take-profit band (0.25%) — protects a gapping mark from
 *  filling a take-profit far through its trigger. */
export const MARK_TP_SLIPPAGE_BPS = 25n;
/** Wide default stop-loss band (10%) — wide so a stop still FIRES in a fast
 *  move (a too-tight SL floor would make the stop silently never fill). */
export const MARK_SL_SLIPPAGE_BPS = 1000n;
/** Max rungs per laddered exit (e.g. up to 4 TP + 1 SL) — bounds rent
 *  (~0.002 SOL/Order PDA) and the keeper's per-tick order-set growth. */
export const MAX_EXIT_RUNGS = 5;

const BPS_DENOM = 10000n;

/**
 * Resolve a rung's on-chain `min_output_usdc` floor (Spec #15 §B.3).
 *   - explicit `rung.minOutputUsdc` wins (incl. `0n` to disable);
 *   - else, when `expectedNetPayoutUsdc` is supplied, apply the default band:
 *       floor = expectedNetPayout × (1 − bps/10000),  bps = TP 25 / SL 1000;
 *   - else `0n` (no floor — today's behavior, keeps the default-OFF path
 *     byte-identical).
 */
export function resolveExitMinOutput(rung: ExitRung, isTakeProfit: boolean): bigint {
  if (rung.minOutputUsdc !== undefined) return rung.minOutputUsdc;
  if (rung.expectedNetPayoutUsdc !== undefined && rung.expectedNetPayoutUsdc > 0n) {
    const bps = isTakeProfit ? MARK_TP_SLIPPAGE_BPS : MARK_SL_SLIPPAGE_BPS;
    return (rung.expectedNetPayoutUsdc * (BPS_DENOM - bps)) / BPS_DENOM;
  }
  return 0n;
}
...
export class PerpClient {
  ...
  async openPositionIx(
    accounts: {
      ...
      signer:               PublicKey;
      /** Position owner (WP-1 owner/signer split). Defaults to `signer` for
       *  non-delegated opens; pass the wallet owner when `signer` is a delegate. */
      owner?:               PublicKey;
    ...
    // WP-1: the position owner defaults to the signer (non-delegated). The
    // credit_account PDA is seeded by the OWNER, not the delegate.
    const ownerKey = accounts.owner ?? accounts.signer;
    const credit = this.resolveCreditAccounts(accounts.poolProgram, ownerKey, {
    ...
    const {
      ...
      feeSettings: _fs, owner: _ownerIgnored,
    ...
    return this.program.methods
      ...
      .accounts({
        ...
        // #12 builder-fee — DARK at launch: system_program sentinel = no builder
        // (resolve_builder_fee returns 0); builder_reward is Option<> → null (None).
        builder:             systemProgram,
        builderRegistry:     systemProgram,
        builderReward:       null,
  ...
  async closePositionIx(
    ...
    return this.program.methods
      ...
      .accounts({
        ...
        affiliate:           affiliate           ?? systemProgram,
        affiliateReward:     affiliateReward     ?? null,
        // #12 builder-fee — DARK at launch: system_program sentinel = no builder; builder_reward Option<> → null.
        builder:             systemProgram,
        builderRegistry:     systemProgram,
        builderReward:       null,
        creditTreasury:      credit.creditTreasury,
  ...
  /**
   * Laddered-exit builder (Spec #15 §4B) — fans a rung list out into N
   * decrease-order ix, each its own Order PDA (nonce++) at a distinct
   * trigger_price/size. A primitive: it derives nothing from chain state, so
   * the caller supplies `startNonce` (the current OrderNonce value),
   * `positionNonce`, `positionSize`, `marketId`, and the resolved `category`
   * arg — exactly as the on-open / PositionDetail paths already do. It wraps
   * the existing `createOrderIx` per rung so the on-chain partial path
   * (§4A `is_full`/proportional slicing) settles each rung independently.
   *
   * Invariants asserted:
   *   - `rungs.length <= MAX_EXIT_RUNGS`
   *   - `sum(rung.sizeUsdc) <= positionSize` (caller-supplied live size)
   * Per rung:
   *   - `collateralUsdc: 0n` (reduce-only — takes no margin)
   *   - `nonce = startNonce + i` (monotonic; the PDA is derived from
   *     `[b"order", owner, marketId, nonce_le8]`, matching the nonce convention
   *     the order-form already uses)
   *   - the SAME `category` arg threaded to EVERY rung (a missed category arg
   *     on any rung reverts WrongPool/WrongMarketRisk; §B.4)
   *   - `min_output_usdc` floor resolved from `resolveExitMinOutput(...)`:
   *     an explicit `rung.minOutputUsdc` wins (incl. `0n` to disable); else the
   *     default band (tight TP / wide SL) is applied against the rung's
   *     `expectedNetPayoutUsdc` when supplied; absent both, `0n` (today's
   *     no-floor behavior — the default-OFF path stays byte-identical).
   *
   * `order.side` is the POSITION side, NOT the execution side (load-bearing —
   * the on-chain match table at execute_order.rs:225-235 reads it that way).
   * The builder carries `positionSide` to every rung verbatim; do NOT flip.
   */
  async buildLadderedExitIxs(
    common: {
      marketState: PublicKey;
      poolState:   PublicKey;   // category pool when repointed, else per-market
      orderNonce:  PublicKey;   // PDA [b"order_nonce", owner]
      /** Signer of the tx — owner (self) or a registered delegate. */
      signer?:     PublicKey;
      owner:       PublicKey;
      tradingKey?: PublicKey | null;
      ownerUsdc:   PublicKey;
      vaultUsdc:   PublicKey;   // category vault when repointed, else per-market
      poolProgram: PublicKey;
      engineAuth:  PublicKey;   // legacy per-market engine_auth PDA (named)
      /** Market id — used to derive each rung's Order PDA
       *  ([b"order", owner, marketId, (startNonce+i)_le8]). */
      marketId:    Uint8Array;
    },
    args: {
      /** Parent position side — carried verbatim to each rung (do NOT flip). */
      positionSide: { long: {} } | { short: {} };
      orderType:    { limitDecrease: {} } | { stopLossDecrease: {} };
      /** Live parent position size (sum(rung.sizeUsdc) must be <= this). */
      positionSize: bigint;
      positionNonce: bigint;
      /** Current OrderNonce value; rung i takes startNonce + i. */
      startNonce:   bigint;
      rungs:        ExitRung[];
      referralCode: number[];   // Array(32).fill(0) = no referral
    },
    /** Resolved category arg (engine_auth), threaded to every rung — pass ONLY
     *  for a CategoryPool-repointed market; accepts the broader categoryBuilderArg. */
    category?: { engineAuth: PublicKey },
  ): Promise<TransactionInstruction[]> {
    const { rungs, orderType, positionSide, positionSize, positionNonce, startNonce, referralCode } = args;
    if (rungs.length === 0) {
      throw new Error("buildLadderedExitIxs: at least one rung is required");
    }
    if (rungs.length > MAX_EXIT_RUNGS) {
      throw new Error(
        `buildLadderedExitIxs: ${rungs.length} rungs exceeds MAX_EXIT_RUNGS (${MAX_EXIT_RUNGS})`,
      );
    }
    const sumSize = rungs.reduce((acc, r) => acc + r.sizeUsdc, 0n);
    if (sumSize > positionSize) {
      throw new Error(
        `buildLadderedExitIxs: sum(sizeUsdc)=${sumSize} exceeds positionSize=${positionSize}`,
      );
    }
    const isTp = "limitDecrease" in orderType;
    const ixs: TransactionInstruction[] = [];
    for (let i = 0; i < rungs.length; i++) {
      const rung = rungs[i];
      const nonce = startNonce + BigInt(i);
      const [orderPubkey] = orderPDA(
        common.owner,
        common.marketId,
        nonce,
        this.program.programId,
      );
      const minOutputUsdc = resolveExitMinOutput(rung, isTp);
      const ix = await this.createOrderIx(
        {
          marketState: common.marketState,
          poolState:   common.poolState,
          orderNonce:  common.orderNonce,
          order:       orderPubkey,
          signer:      common.signer,
          owner:       common.owner,
          tradingKey:  common.tradingKey,
          ownerUsdc:   common.ownerUsdc,
          vaultUsdc:   common.vaultUsdc,
          poolProgram: common.poolProgram,
          engineAuth:  common.engineAuth,
        },
        {
          orderType,
          side:            positionSide,   // POSITION side, verbatim — do NOT flip
          sizeUsdc:        rung.sizeUsdc,
          collateralUsdc:  0n,             // reduce-only — no margin
  ...
  async createOrderIx(
    accounts: {
      ...
      /** Signer of the tx — the owner (self) or a registered delegate.
       *  Defaults to `owner` for non-delegated creates (WP-1 owner/signer split). */
      signer?:     PublicKey;
      owner:       PublicKey;
      /** TradingKey PDA [b"trading_key", owner] when `signer` is a delegate; null/undefined
       *  for self-create (Anchor 0.31 optional account — null is valid at runtime). */
      tradingKey?: PublicKey | null;
      ownerUsdc:   PublicKey;
      vaultUsdc:   PublicKey;
      poolProgram: PublicKey;
    ...
    const systemProgram = new PublicKey("11111111111111111111111111111111");
    // WP-1: signer defaults to owner (self-create); tradingKey null = no delegation.
    const { signer, tradingKey, ...rest } = accounts;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const builder = (this.program.methods.createOrder as any)({
      ...
      .accounts({
        ...rest,
        signer:        signer ?? accounts.owner,
        tradingKey:    (tradingKey ?? null) as any,   // Anchor 0.31 optional account
        tokenProgram:  TOKEN_PROGRAM_ID,
        systemProgram,
      });
    if (category) {
  ...
  async cancelOrderIx(
    accounts: {
      ...
      signer: PublicKey;
      /** TradingKey PDA [b"trading_key", owner] when `signer` is a delegate; null/undefined
       *  for self-cancel (Anchor 0.31 optional account — null is valid at runtime). */
      tradingKey?: PublicKey | null;
      owner: PublicKey;
      ownerUsdc: PublicKey;
      vaultUsdc: PublicKey;
  ...
  async updateMarketConfigIx(
    ...
    args: {
      ...
      liqFeeBps?: number | null;
      partialLiqTargetHealth?: number | null;
      // Phoenix #1 off-hours execution band (2026-06-24): 0 = disabled; non-zero
      // 50..5000 bps (validated on-chain). u16 — plain number, NOT BN.
      offHoursBandBps?: number | null;
      // Phoenix #9 asymmetric uPnL withdrawal haircut (2026-06-24): 0 = no haircut
      // (default / kill-switch); non-zero <= 10000. u16 — plain number, NOT BN.
      upnlRiskFactorBps?: number | null;
    },
  ): Promise<TransactionInstruction> {
    ...
    return this.program.methods
      .updateMarketConfig({
        ...
        // Anchor omits absent Options → a partial object under-serializes against
        // the deserializer and the tx reverts, so EVERY field present on the
        // on-chain UpdateMarketConfigArgs must be present here (default null).
        liqFeeBps: args.liqFeeBps ?? null,
        partialLiqTargetHealth: args.partialLiqTargetHealth ?? null,
        offHoursBandBps: args.offHoursBandBps ?? null,   // Phoenix #1
        upnlRiskFactorBps: args.upnlRiskFactorBps ?? null, // Phoenix #9
      })
      .accounts(accounts)
      .instruction();
  }
  ...
  /**
   * Build an `update_protocol_config` instruction. All args are optional
   * (`Option<T>` on-chain); pass `null` to leave a field unchanged. Includes the
   * Phoenix #12 `maxBuilderFeeBps` builder-fee cap (0 = builder fees OFF
   * venue-wide / default; non-zero ≤ `MAX_BUILDER_FEE_BPS_CAP` = 100 on-chain).
   *
   * `protocol_config` (`[b"protocol_config"]`) is auto-resolved by Anchor from its
   * const seed — only the `admin` signer is required.
   */
  async updateProtocolConfigIx(
    accounts: { admin: PublicKey },
    args: {
      rthOpenMinutesUtc?: number | null;
      rthCloseMinutesUtc?: number | null;
      tierBreakpointsUsdc?: bigint[] | null;
      rthMaxLeverage?: number[] | null;
      offHoursMaxLeverage?: number[] | null;
      minFreeLiquidityBps?: number | null;
      // Phoenix #12 builder-fee cap (u16, bps). Plain number, NOT BN.
      maxBuilderFeeBps?: number | null;
    },
  ): Promise<TransactionInstruction> {
    return this.program.methods
      .updateProtocolConfig({
        rthOpenMinutesUtc: args.rthOpenMinutesUtc ?? null,
        rthCloseMinutesUtc: args.rthCloseMinutesUtc ?? null,
        tierBreakpointsUsdc:
          args.tierBreakpointsUsdc != null
            ? args.tierBreakpointsUsdc.map((v) => new BN(v.toString()))
            : null,
        rthMaxLeverage: args.rthMaxLeverage ?? null,
        offHoursMaxLeverage: args.offHoursMaxLeverage ?? null,
        minFreeLiquidityBps: args.minFreeLiquidityBps ?? null,
        maxBuilderFeeBps: args.maxBuilderFeeBps ?? null, // Phoenix #12
      })
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      .accounts({ admin: accounts.admin } as any)
      .instruction();
  }
  ...
  /**
   * Build a `migrate_market_state_v5` instruction (Phoenix #1 + #9, 2026-06-24):
   * the one-shot 515 → 527 realloc that appends `off_hours_band_bps` (2B @515),
   * `last_external_anchor` (8B @517) and `upnl_risk_factor_bps` (2B @525), all
   * zero-init (inert). Idempotent (no-op if already 527); strict (only 515 → 527).
   * MUST run venue-wide immediately after the perp redeploy that ships the
   * 527-byte struct, before any market is read as v5.
   *
   * `market_state` (`[b"market", market_id]`) and `protocol_config`
   * (`[b"protocol_config"]`) are auto-resolved by Anchor from `marketId` + the
   * const seed — only the `admin` signer is required.
   */
  async migrateMarketStateV5Ix(
    accounts: { admin: PublicKey },
    args: { marketId: Uint8Array },
  ): Promise<TransactionInstruction> {
    const systemProgram = new PublicKey("11111111111111111111111111111111");
    return this.program.methods
      .migrateMarketStateV5(Array.from(args.marketId))
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      .accounts({ admin: accounts.admin, systemProgram } as any)
      .instruction();
  }

  /**
   * Build a `set_external_anchor` instruction (Phoenix #1, 2026-06-24): the
   * feed-admin-gated stamp of `MarketState.last_external_anchor` — the
   * last-clean-RTH-close the off-hours execution band bounds fills against.
   *
   * Signed by the secondary `PriceFeed.admin` (the relayer key, `9Zip…`); the
   * passed `priceFeed` account is bound to the market by its `market_id`
   * (bytes 40..72) == `marketId`. `price` is the anchor in the oracle-adapter's
   * NORMALIZED space (NOT the raw feed mantissa — see the spec §10 anchor-encoding
   * note) and must be > 0 (0 disables the band — use `update_market_config
   * { offHoursBandBps: 0 }` to disable instead).
   *
   * `market_state` (`[b"market", market_id]`) is auto-resolved; pass the
   * secondary `priceFeed` PDA and the `feedAdmin` signer.
   */
  async setExternalAnchorIx(
    accounts: { priceFeed: PublicKey; feedAdmin: PublicKey },
    args: { marketId: Uint8Array; price: bigint },
  ): Promise<TransactionInstruction> {
    return this.program.methods
      .setExternalAnchor(Array.from(args.marketId), new BN(args.price.toString()))
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      .accounts({
        priceFeed: accounts.priceFeed,
        feedAdmin: accounts.feedAdmin,
      } as any)
      .instruction();
  }

  /**
   * Build a `register_builder` instruction (Phoenix #12, 2026-06-24): self-register
   * a builder identity at `[b"builder", authority]` with a chosen surcharge bps
   * (clamped on-chain to `ProtocolConfig.max_builder_fee_bps`). Inert until the
   * operator arms the cap AND the SDK threads this builder's PDAs.
   *
   * `builder_registry` (`[b"builder", authority]`) and `protocol_config` are
   * auto-resolved; pass the `authority` signer (becomes the registry authority
   * and the claim signer).
   */
  async registerBuilderIx(
    accounts: { authority: PublicKey },
    args: { feeBps: number },
  ): Promise<TransactionInstruction> {
    const systemProgram = new PublicKey("11111111111111111111111111111111");
    return this.program.methods
      .registerBuilder(args.feeBps)
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      .accounts({ authority: accounts.authority, systemProgram } as any)
      .instruction();
  }

  /**
   * Build an `update_builder_fee` instruction (Phoenix #12): the builder updates
   * its own surcharge bps (re-clamped to the on-chain cap). Gated to the registry's
   * own `authority`. PDAs auto-resolve from the `authority` signer.
   */
  async updateBuilderFeeIx(
    accounts: { authority: PublicKey },
    args: { feeBps: number },
  ): Promise<TransactionInstruction> {
    return this.program.methods
      .updateBuilderFee(args.feeBps)
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      .accounts({ authority: accounts.authority } as any)
      .instruction();
  }

  /**
   * Build a `claim_builder_reward` instruction (Phoenix #12): claim a builder's
   * accrued per-market surcharge from the pool vault (where it was earmarked OUT
   * of LP NAV) via a pool-program `disburse_builder_earmark[_category]` CPI.
   * Category-aware; engine_auth-signed.
   *
   * `builder_reward` (`[b"builder_reward", builder, market_id]`), `market_state`
   * (`[b"market", market_id]`) and `engine_auth` are auto-resolved from
   * `market_state.market_id`; pass the remaining accounts explicitly. For a
   * CategoryPool-backed market, append the category `engine_auth` as the single
   * remaining account (the `CATEGORY_ENGINE_AUTH_RA_INDEX` slot).
   */
  async claimBuilderRewardIx(
    accounts: {
      builderReward: PublicKey;
      marketState: PublicKey;
      builder: PublicKey;
      builderUsdc: PublicKey;
      poolState: PublicKey;
      poolProgram: PublicKey;
      usdcVault: PublicKey;
      vaultAuthority: PublicKey;
