...
export class PoolClient {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // Phoenix-borrow #7 / #12 — withdrawal rate-limiter + builder-fee earmark
  // (CategoryPool 264/290→298, PoolState 260→268). Migrations are admin-gated,
  // idempotent, and INERT (bucket seeded DISABLED, earmark seeded 0). NOTE:
  // withdrawCategoryIx above needs NO account change — the #7 bucket lives on
  // the CategoryPool that withdraw_category already passes.
  // ─────────────────────────────────────────────────────────────────────────

  /**
   * Build a migrate_category_pool_wd_bucket instruction (#7/#12). Grows a live
   * CategoryPool 264→298 (also accepts a 290-byte #7-only pool and grows it to
   * 298), seeding the withdrawal token bucket DISABLED (wd_max_budget_bps_of_nav
   * = 0 ⇒ wd_max_budget() == u64::MAX, available = u64::MAX) and the builder
   * earmark = 0 — fully inert (lp_principal() byte-identical). Idempotent: a pool
   * already at 298 is a no-op. The CategoryPool is an UncheckedAccount on-chain
   * (manual realloc), so pass `categoryId` as the arg and the resolved PDA.
   */
  async migrateCategoryPoolWdBucketIx(
    accounts: {
      categoryPool: PublicKey;
      admin: PublicKey;
    },
    args: { categoryId: Uint8Array },
  ): Promise<TransactionInstruction> {
    return this.program.methods
      .migrateCategoryPoolWdBucket(Array.from(args.categoryId))
      .accounts({
        categoryPool: accounts.categoryPool,
        admin: accounts.admin,
        systemProgram: SystemProgram.programId,
      })
      .instruction();
  ...
  /**
   * Build an update_category_wd_bucket instruction (#7, operator-facing). Arms or
   * disables the per-pool withdrawal rate-limiter. `maxBudgetBpsOfNav = 0` is the
   * primary kill-switch (limiter OFF). A non-zero value (capped at 5000 = 50%
   * NAV/window) caps a withdraw at bps% of lp_principal() per refill window.
   * `refillPerSlot` = micro-USDC added per elapsed slot. `resetAvailableFull`
   * un-throttles now (sets available = wd_max_budget()) without disabling.
   * Pass null to leave a field unchanged (Option<T> on-chain).
   */
  async updateCategoryWdBucketIx(
    accounts: {
      categoryPool: PublicKey;
      admin: PublicKey;
    },
    args: {
      maxBudgetBpsOfNav?: number | null;
      refillPerSlot?: bigint | null;
      resetAvailableFull?: boolean;
    },
  ): Promise<TransactionInstruction> {
    return this.program.methods
      .updateCategoryWdBucket({
        maxBudgetBpsOfNav: args.maxBudgetBpsOfNav ?? null,
        refillPerSlot: args.refillPerSlot != null ? new BN(args.refillPerSlot.toString()) : null,
        resetAvailableFull: args.resetAvailableFull ?? false,
      })
      .accounts({
        categoryPool: accounts.categoryPool,
        admin: accounts.admin,
  ...
  /**
   * Build a migrate_pool_state_v4 instruction (#12, legacy PoolState 260→268).
   * Grows a live collateral-escrow-layout (260-byte) PoolState to 268, seeding
   * builder_earmark_usdc = 0 — inert (lp_principal() byte-identical). Idempotent:
   * a pool already at 268 is a no-op. The PoolState is an UncheckedAccount
   * on-chain (manual realloc), so pass `marketId` as the arg and the resolved PDA.
   */
  async migratePoolStateV4Ix(
    accounts: {
      poolState: PublicKey;
      admin: PublicKey;
    },
    ...
    return this.program.methods
      .migratePoolStateV4(Array.from(args.marketId))
      .accounts({
        poolState: accounts.poolState,
        admin: accounts.admin,
        systemProgram: SystemProgram.programId,
