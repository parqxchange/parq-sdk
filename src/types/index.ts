...
export interface MarketState {
  ...
  // V5 append (LEN 515 → 527) — undefined when decoding a V1..V4 buffer.
  // Phoenix #1 off-hours execution band + #9 asymmetric uPnL withdrawal haircut
  // (2026-06-24). All three ship inert (0); behavior is per-market opt-in.
  offHoursBandBps?:                 number;      // u16 — #1 off-hours band width (0 = disabled)
  lastExternalAnchor?:              bigint;      // u64 — #1 relayer-stamped last-RTH-close (0 = unset)
  upnlRiskFactorBps?:               number;      // u16 — #9 uPnL haircut factor (0 = no haircut, ≤10000)
}
...
// V5 borsh: 519 bytes + 8 discriminator = 527 (off_hours_band_bps 2B @515 + last_external_anchor 8B @517 + upnl_risk_factor_bps 2B @525)
...
export interface PoolState {
  ...
  // Collateral-escrow (v6 — appended by migrate_pool_state_v3, 2026-06-12).
  // Trader collateral the pool holds in its vault but does NOT own; LP NAV must
  // exclude it. 0 on pre-v6 (240/252-byte) pools.
  escrowedCollateralUsdc: bigint;  // u64
  // Builder-fee earmark (v7/#12 — appended by migrate_pool_state_v4, 2026-06-24).
  // Builder-owed cash held in the vault but excluded from LP NAV (lp_principal
  // subtracts it). Optional: 0 / undefined on pre-v7 (240/252/260-byte) pools.
  builderEarmarkUsdc?: bigint;     // u64 @260
}
// Total borsh: 232 bytes + 8 discriminator = 240 (v4); v5 appends u32+u64 = 252;
// v6 appends u64 = 260; v7 appends u64 = 268.
...
// v6: escrowedCollateralUsdc appended after lastAdlCutIdx.
// v7: builderEarmarkUsdc (#12) appended after escrowedCollateralUsdc @260.

/** Unified-LP-pool category pool (Phase 0/3). LEN 264. */
export interface CategoryPool {
  categoryId:             Uint8Array; // [u8; 32]
  usdcVault:              PublicKey;
  vaultAuthorityBump:     number;
  lpMint:                 PublicKey;
  totalUsdc:              bigint;
  cumulativeFees:         bigint;
  escrowedCollateralUsdc: bigint;
  sumReservedUsdc:        bigint;
  sumQueueOwedUsdc:       bigint;
  ...
  bump:                   number;
  sumSideBucketUsdc:      bigint;     // appended after bump (#202 category NAV)
  // Withdrawal rate-limiter token bucket (#7 — appended @264..290 by
  // migrate_category_pool_wd_bucket, 2026-06-24). Optional: undefined on the
  // pre-#7 (264-byte) layout. Limiter DISABLED (inert) when
  // wdMaxBudgetBpsOfNav == 0 (→ wd_max_budget() == u64::MAX).
  wdBucketAvailable?:     bigint;     // u64 @264 — current drainable budget (USDC micro-units)
  wdBucketLastSlot?:      bigint;     // u64 @272 — last lazy-refill slot
  wdMaxBudgetBpsOfNav?:   number;     // u16 @280 — 0 = DISABLED; else cap = bps * lp_principal / 10_000
  wdRefillPerSlot?:       bigint;     // u64 @282 — micro-USDC added per elapsed slot
  // Builder-fee earmark (#12 — appended @290..298 by migrate_category_pool_wd_bucket
  // and migrate_pool_state_v4 path, 2026-06-24). Optional: undefined on the
  // pre-#12 (264/290-byte) layouts. Builder-owed cash ⊆ total_usdc, excluded
  // from LP NAV (lp_principal subtracts it). 0 at dark-launch.
  builderEarmarkUsdc?:    bigint;     // u64 @290
}

/** Per-market risk record bound to a category (Phase 0/3). LEN 199. */
export interface MarketRisk {
  ...
  bump:                  number;
  migrationOldLpSupply:  bigint;     // Phase 3 convert_lp snapshot ratio (denominator)
  migrationEscrowMinted: bigint;     // Phase 3 convert_lp snapshot ratio (numerator)
}
 ...
 *   pauseAuthority             Pubkey       (32)  // #127; appended (LEN 126 → 158); default = unset
 *
...
export interface ProtocolConfig {
  ...
  maxBuilderFeeBps?:    number;     // u16: #12 builder-fee cap (carved from _reserved, decodes 0 pre-arm). Optional — old fetches that don't surface it default to off.
  ...
  pauseAuthority?:      PublicKey;  // #127: least-privilege keeper pause signer; default (all-zeros) = unset/disabled. Optional — pre-V3 (126-byte) accounts decode without it.
}
...
// #12 builder-fee primitive (Phoenix, 2026-06-24). Net-new accounts; the
// non-builder hot path passes system-program sentinels so these are inert/dark
// until the operator arms `ProtocolConfig.maxBuilderFeeBps` (default 0).
export interface BuilderRegistry {
  authority: PublicKey;   // Pubkey — builder's signing wallet / claim signer
  feeBps:    number;      // u16 — chosen surcharge bps (clamped to the on-chain cap)
  bump:      number;      // u8
}
// Total borsh: 35 bytes + 8 discriminator = 43; seeds [b"builder", authority]

export interface BuilderReward {
  builder:     PublicKey;   // Pubkey
  marketId:    Uint8Array;  // [u8; 32]
  accruedUsdc: bigint;      // u64
  bump:        number;      // u8
}
// Total borsh: 73 bytes + 8 discriminator = 81 (verbatim AffiliateReward shape);
// seeds [b"builder_reward", builder, market_id]
...
export interface LiquidatedEvent {
  ...
  marketId: Uint8Array;
  badDebt:  bigint;
  /** Notional closed this liquidation (6-dec USDC). 0 for pre-Phase-2 events. */
  closedSize:    bigint;
  /** Survivor size; 0 ⇒ full close (and for pre-Phase-2 events). */
  remainingSize: bigint;
  /** Equity-denominated total fee (liquidator + insurance). 0 for pre-Phase-2 events. */
  feePaid:       bigint;
}
...
// Emitted by `harvest` when a Pending winner is paid (wholly or
