...
export enum ParquetError {
  ...
  CannotRemoveQueueCollateral = 8017,
  ...
  TradingKeyExpiryTooLong = 10007,
  QueueDrawWhileDelegated = 10008,
...
const ERROR_MESSAGES: Partial<Record<ParquetError, string>> = {
  ...
  [ParquetError.CannotRemoveQueueCollateral]: "Cannot remove queue-sourced collateral; it can only exit via close",
  ...
  [ParquetError.TradingKeyExpiryTooLong]: "Trading key expiry exceeds the maximum allowed TTL",
  [ParquetError.QueueDrawWhileDelegated]: "A delegate may not draw queue collateral (wallet-only when delegated)",
